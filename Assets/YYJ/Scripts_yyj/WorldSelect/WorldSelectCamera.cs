...
public class WorldSelectCamera : MonoBehaviour
{
    ...
    private WorldNode currentFocusNode;
    ...
    void Start()
    {
        ...
        WorldNode[] allNodes = FindObjectsOfType<WorldNode>();
    ...
    void HandleClick()
    {
        ...
        {
            WorldNode clickedNode = hit.collider.GetComponent<WorldNode>();
            if (clickedNode != null)
            {
    ...
    // 이동
    void FocusOnNode(WorldNode targetNode)
    {
    ...
    */
    // 확대
    void ZoomToNode(WorldNode node)
    {
        Bounds bounds = node.GetComponent<SpriteRenderer>().bounds;
